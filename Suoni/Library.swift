...
class Conversione {
    var libreria : [SoundID] = []
    init() {
        carica()
    }
    func carica(){
//        leggo i dizionario con valore
        for lib in library["category"]! {
//            leggo il value 
            for i in lib.value {
                libreria.append(SoundID(category: lib.key, fileName: i.key , iD: i.value))
            }
        }
    }
}
